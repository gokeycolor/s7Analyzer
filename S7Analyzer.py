...
# 默认值
SLICE_PKT_DEFAULT = 10_000  # 每片报文数（可微调）
...
def main():
    parser = argparse.ArgumentParser(description="Process a pcap file and extract S7 variables to a CSV file.",
                                     epilog="Example usage: s7Analyzer.exe -i input.pcap -o output.csv -s 5000")
    parser.add_argument("-i", "--input", required=True, help="Input pcap or pcapng file path (required)")
    parser.add_argument("-o", "--output", required=True, help="Output CSV file path (required)")
    parser.add_argument("-s", "--slice", type=int, default=SLICE_PKT_DEFAULT,
                        help=f"Number of packets per slice (default: {SLICE_PKT_DEFAULT})")

    args = parser.parse_args()

    PCAP_FILE = args.input
    CSV_FILE = args.output
    SLICE_PKT = args.slice

    if not os.path.isfile(PCAP_FILE):
        print(f"Error: The input file '{PCAP_FILE}' does not exist.")
        parser.print_help()
        return
